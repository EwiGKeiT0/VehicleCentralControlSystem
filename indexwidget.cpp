    ...
    , mainWindow(father)
{
    ...
    setGradient(ui->querybutton, QColor(72, 72, 84), QColorConstants::Svg::lightblue, 150);
    setGradient(ui->location, QColor(72, 72, 84), QColorConstants::Svg::lightcyan, 150);
    setGradient(ui->searchbox, QColor(72, 72, 84), QColorConstants::Svg::darkgrey, 150);

    init_ip();
    get_ip();
    while (ip.size() > 0) {
        qDebug() << ip << "ipipipipipipipipip";
    }
    //图例初始化
    init_pic();
    //搜索框
    suggest();
    // connect(this, &indexWidget::readready, this, &indexWidget::test);
...
QString indexWidget::get_loc() {
    ...
    QString url = "https://api.map.baidu.com/location/ip?ip=";
    url+=ip+"&coor=bd09ll&ak=CDE9axcE8tPnZZRLiPdgK8IIsXOJ77a3";
    ...
    connect(reply, &QNetworkReply::readyRead, [=](){
        ...
        QJsonObject addressobj = contentobj["address_detail"].toObject();
...
void indexWidget::onGetIp(QNetworkReply *reply){
    //reply->readAll()  服务响响应的正文内容
    //qDebug() << "Current IP:" << reply->readAll();
    QJsonDocument jsonDoc = QJsonDocument::fromJson(reply->readAll());
    QJsonObject jsonObj = jsonDoc.object();

    ip=jsonObj.value("origin").toString();
    qDebug() << "Current IP:" << ip;
}

void indexWidget::get_ip(){
    //声明url 请求地址
...
void indexWidget::onQueryClicked() {
    ...
    completer = new QCompleter(emptyList, ui->searchbox);
...
void indexWidget::parse_weatherdata2(QString targetSite) {
    datalist.clear();
    QNetworkAccessManager *manager = new QNetworkAccessManager(this);
    QNetworkRequest request;
...
void indexWidget::on_location_clicked()
{
    flag = 1;
    parse_weatherdata();
}
